...
window.NfcCallbacks={
  ...
  onLockState:value=>window.__onLockState&&window.__onLockState(value),
  onUnlockResult:value=>window.__onUnlockResult&&window.__onUnlockResult(value)
};

function LockScreen({error,onUnlock}){return <div className="lock-screen"><div role="dialog" aria-modal="true" aria-labelledby="lock-title" style={{display:"flex",flexDirection:"column",alignItems:"center"}}>
  <div className="lock-mark" aria-hidden="true">N</div>
  <h1 id="lock-title">Vault locked</h1>
  <p>Confirm your device PIN, pattern, or password to open your encrypted vault.</p>
  <button className="primary" onClick={onUnlock}>Unlock</button>
  {error&&<div className="notice error-notice" role="alert" style={{marginTop:18,maxWidth:340}}>{error}</div>}
</div></div>}
...
function Guide({prefs,setPrefs,lock,setLock}){const matrix=[
  ...
  <aside className="grid"><div className="panel pad"><div className="section-title"><h2>Display preferences</h2></div>{[["largeText","Larger text","Increase text throughout the app."],["highContrast","High contrast","Strengthen borders and secondary text."],["reduceMotion","Reduce motion","Minimize scanning and transition animation."]].map(([key,title,text])=><div className="switch-row" key={key}><div><strong>{title}</strong><div className="hint">{text}</div></div><button className={"switch "+(prefs[key]?"on":"")} role="switch" aria-checked={prefs[key]} aria-label={title} onClick={()=>setPrefs({...prefs,[key]:!prefs[key]})}><span/></button></div>)}</div>{nativeBridge&&<div className="panel pad"><div className="section-title"><h2>App lock</h2></div><div className="switch-row"><div><strong>Require unlock</strong><div className="hint">{lock.deviceSecure?"Ask for your device PIN, pattern, or password before showing the vault.":"Set a screen lock in your Android settings to turn this on."}</div></div><button className={"switch "+(lock.enabled?"on":"")} role="switch" aria-checked={lock.enabled} aria-label="Require unlock" disabled={!lock.deviceSecure} onClick={()=>setLock(!lock.enabled,lock.timeout)}><span/></button></div>{lock.enabled&&<><label className="form-label" htmlFor="lock-timeout">Lock after</label><select id="lock-timeout" className="field" value={lock.timeout} onChange={e=>setLock(true,Number(e.target.value))}>{[[0,"Immediately"],[60,"1 minute"],[300,"5 minutes"],[900,"15 minutes"]].map(([v,l])=><option value={v} key={v}>{l}</option>)}</select><div className="hint" style={{marginTop:6}}>Locks after NFC Vault spends this long in the background, and every time the app restarts.</div></>}</div>}<div className="panel pad"><div className="section-title"><h2>Privacy and safety</h2></div><p className="hint">Vault data is encrypted locally with a key held by Android Keystore. Backups are disabled. The app works offline and does not request internet access.</p><div className="notice good" style={{marginTop:14}}>NFC Vault shares only standard NDEF records. It does not emulate payment cards or bypass protected credentials.</div></div></aside></div></>}
...
function App(){
  ...
  const [locked,setLocked]=useState(()=>{try{return nativeBridge?window.NfcBridge.isLocked():false}catch(e){return false}}),[lockError,setLockError]=useState(null),[lockCfg,setLockCfg]=useState({enabled:false,timeout:0,deviceSecure:false});
  const importRef=useRef(null),toastRef=useRef(null);
  const hydrateCards=()=>{let loaded=loadValue(CARD_KEY,[]);if(!Array.isArray(loaded)||loaded.length===0){const legacy=loadValue(LEGACY_CARD_KEY,[]);if(Array.isArray(legacy)&&legacy.length){loaded=legacy;saveValue(CARD_KEY,legacy);if(nativeBridge)window.NfcBridge.removeData(LEGACY_CARD_KEY);else localStorage.removeItem(LEGACY_CARD_KEY)}}setCards(Array.isArray(loaded)?loaded.slice(0,500).map(normalizeCard):[])};
  ...
  useEffect(()=>{const p=loadValue(PREF_KEY,{});setPrefs(old=>({...old,...p}));if(nativeBridge){try{if(!window.NfcBridge.isNfcAvailable())setStatus("unavailable");else setStatus(window.NfcBridge.isNfcEnabled()?"ready":"disabled");setHce(window.NfcBridge.isEmulationAvailable());const cfg=safeParse(window.NfcBridge.getLockConfig(),{});setLockCfg({enabled:!!cfg.enabled,timeout:cfg.timeout||0,deviceSecure:!!cfg.deviceSecure})}catch(e){setStatus("unavailable")}}},[]);
  useEffect(()=>{if(!locked)hydrateCards()},[locked]);
  useEffect(()=>{window.__onLockState=state=>{const isLocked=state==="locked";setLocked(isLocked);if(!isLocked)setLockError(null)};window.__onUnlockResult=result=>{if(result!=="success")setLockError(result==="cancelled"?"Authentication cancelled. Tap Unlock to try again.":"Could not authenticate. Tap Unlock to try again.")};return()=>{window.__onLockState=null;window.__onUnlockResult=null}},[]);
  ...
  const setLock=(enabled,timeout)=>{if(!nativeBridge)return;window.NfcBridge.setLockConfig(enabled,timeout);const cfg=safeParse(window.NfcBridge.getLockConfig(),{});setLockCfg({enabled:!!cfg.enabled,timeout:cfg.timeout||0,deviceSecure:!!cfg.deviceSecure})};
  ...
  if(locked)return <LockScreen error={lockError} onUnlock={()=>{setLockError(null);if(nativeBridge)window.NfcBridge.requestUnlock()}}/>;
